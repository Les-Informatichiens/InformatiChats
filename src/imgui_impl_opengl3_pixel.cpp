...
//  2019-09-16: OpenGL: Tweak initialization code to allow application calling ImGui_ImplOpenGL3_Pixel_CreateFontsTexture() before the first NewFrame() call.
//  2019-05-29: OpenGL: Desktop GL only: Added support for large mesh (64K+ vertices), enable ImGuiBackendFlags_RendererHasVtxOffset flag.
//  2019-04-30: OpenGL: Added support for special ImDrawCallback_ResetRenderState callback to reset render state.
//  2019-03-29: OpenGL: Not calling glBindBuffer more than necessary in the render loop.
//  2019-03-15: OpenGL: Added a GL call + comments in ImGui_ImplOpenGL3_Pixel_Init() to detect uninitialized GL function loaders early.
...
//  2018-03-06: OpenGL: Added const char* glsl_version parameter to ImGui_ImplOpenGL3_Pixel_Init() so user can override the GLSL version e.g. "#version 150".
...
struct ImGui_ImplOpenGL3_Pixel_Data
{
    ...
    ImGui_ImplOpenGL3_Pixel_Data() { memset((void*)this, 0, sizeof(*this)); }
};
...
struct ImGui_ImplOpenGL3_Pixel_VtxAttribState
{
...
bool    ImGui_ImplOpenGL3_Pixel_Init(const char* glsl_version)
{
    ...
    ImGui_ImplOpenGL3_Pixel_Data* bd = IM_NEW(ImGui_ImplOpenGL3_Pixel_Data)();
    ...
    */
    ...
    if (io.ConfigFlags & ImGuiConfigFlags_ViewportsEnable)
        ImGui_ImplOpenGL3_Pixel_InitPlatformInterface();
...
void    ImGui_ImplOpenGL3_Pixel_Shutdown()
{
    ...
    ImGui_ImplOpenGL3_Pixel_ShutdownPlatformInterface();
    ImGui_ImplOpenGL3_Pixel_DestroyDeviceObjects();
...
static void ImGui_ImplOpenGL3_Pixel_SetupRenderState(ImDrawData* draw_data, int fb_width, int fb_height, GLuint vertex_array_object)
{
    ...
    // resize fb texture
//    glBindTexture(GL_TEXTURE_2D, bd->FramebufferTexture);
//    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, fb_width*resFactor, fb_height*resFactor, 0, GL_RGBA, GL_UNSIGNED_BYTE, 0);
//    glBindTexture(GL_TEXTURE_2D, 0);
//    glBindFramebuffer(GL_FRAMEBUFFER, bd->Framebuffer);
...
void    ImGui_ImplOpenGL3_Pixel_RenderDrawData(ImDrawData* draw_data)
{
    ...
    ImGui_ImplOpenGL3_Pixel_VtxAttribState last_vtx_attrib_state_pos; last_vtx_attrib_state_pos.GetState(bd->AttribLocationVtxPos);
    ImGui_ImplOpenGL3_Pixel_VtxAttribState last_vtx_attrib_state_uv; last_vtx_attrib_state_uv.GetState(bd->AttribLocationVtxUV);
    ImGui_ImplOpenGL3_Pixel_VtxAttribState last_vtx_attrib_state_color; last_vtx_attrib_state_color.GetState(bd->AttribLocationVtxColor);
#endif
#ifdef IMGUI_IMPL_OPENGL_USE_VERTEX_ARRAY
...
//    BatchRenderer::drawScreenTex(bd->FramebufferTexture);
...
//    glBindTexture(GL_TEXTURE_2D, bd->FramebufferTexture);
//    BatchRenderer::beginBatch();
////    BatchRenderer::drawTriangle({ -0.5, 100.0 }, { 0.5, 0.5 }, { -1, 0 }, { 255, 255, 255, 255 });
//    BatchRenderer::drawScreenTex(bd->FramebufferTexture);
//    BatchRenderer::endBatch();
//    BatchRenderer::flush();
...
//        "}\n";
    ...
    const GLchar* vertex_shader_glsl_130 = R"(
        precision highp float;
        ...
        void main(void) {
            vColor = aColor;
            vTexCoord = aTexCoord;
            gl_Position = uProjMtx * vec4(aPosition.xy, 0., 1.);
        })";

    const GLchar* fragment_shader_glsl_130 = R"(
        ...
        void main() {
            ...
            if (false)
            {
                ivec2 size = textureSize(uTexture, 0).xy;
                ...
                col.r = 0.7 * texture2D(uTexture, vec2(uv.x + 0.001 * uChroma, uv.y + 0.001 * uChroma)).r;
                col.g = 0.7 * texture2D(uTexture, vec2(uv.x + 0.000 * uChroma, uv.y - 0.002 * uChroma)).g;
                col.b = 0.7 * texture2D(uTexture, vec2(uv.x - 0.002 * uChroma, uv.y + 0.000 * uChroma)).b;

                // Blur
                col += 0.05 * texture2D(uTexture, vec2(uv.x - 2.0 * uBlur / iResolution.x, uv.y));
                col += 0.10 * texture2D(uTexture, vec2(uv.x - 1.0 * uBlur / iResolution.x, uv.y));
                col += 0.10 * texture2D(uTexture, vec2(uv.x + 1.0 * uBlur / iResolution.x, uv.y));
                col += 0.05 * texture2D(uTexture, vec2(uv.x + 2.0 * uBlur / iResolution.x, uv.y));
...
//        fragment_shader = fragment_shader_glsl_410_core;
    ...
    bd->AttribLocationTex = glGetUniformLocation(bd->ShaderHandle, "uTexture");
    bd->AttribLocationProjMtx = glGetUniformLocation(bd->ShaderHandle, "uProjMtx");
    bd->AttribLocationVtxPos = (GLuint)glGetAttribLocation(bd->ShaderHandle, "aPosition");
    bd->AttribLocationVtxUV = (GLuint)glGetAttribLocation(bd->ShaderHandle, "aTexCoord");
    bd->AttribLocationVtxColor = (GLuint)glGetAttribLocation(bd->ShaderHandle, "aColor");

    // Create buffers
    glGenBuffers(1, &bd->VboHandle);
    glGenBuffers(1, &bd->ElementsHandle);

    ImGui_ImplOpenGL3_Pixel_CreateFontsTexture();
...
//    GL_CALL(glGenTextures(1, &bd->FramebufferTexture));
//    GL_CALL(glBindTexture(GL_TEXTURE_2D, bd->FramebufferTexture));
//    GL_CALL(glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, 100, 100, 0, GL_RGBA, GL_UNSIGNED_BYTE, 0));
...
//    glBindTexture(GL_TEXTURE_2D, 0);
//
//    GL_CALL(glGenFramebuffers(1, &bd->Framebuffer));
//    GL_CALL(glBindFramebuffer(GL_FRAMEBUFFER, bd->Framebuffer));
//    GL_CALL(glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, bd->FramebufferTexture, 0));
...
void    ImGui_ImplOpenGL3_Pixel_DestroyDeviceObjects()
{
    ...
    ImGui_ImplOpenGL3_Pixel_DestroyFontsTexture();
}
